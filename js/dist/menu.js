/*!
* Chassis menu.js v0.4.0-0 (https://chassis-ui.com)
...
import { ARROW_DOWN_KEY, ARROW_LEFT_KEY, ARROW_RIGHT_KEY, ARROW_UP_KEY, END_KEY, ENTER_KEY, ESCAPE_KEY, HOME_KEY, SPACE_KEY, TAB_KEY, getElement, getNextActiveElement, isDisabled, isElement, isRTL, isVisible, noop } from "./util/index.js";
...
var Menu = class Menu extends FloatingBase {
	...
	async _applyFloatingPosition(reference, floating, placement, middleware, strategy = "absolute") {
		if (!floating.isConnected || !floating.classList.contains(CLASS_NAME_SHOW)) return null;
	...
	_onSubmenuTriggerClick(event) {
		...
		if (submenu.classList.contains(CLASS_NAME_STACKED)) requestAnimationFrame(() => {
			(SelectorEngine.findOne(SELECTOR_SUBMENU_BACK, submenu) || SelectorEngine.findOne(SELECTOR_VISIBLE_ITEMS, submenu))?.focus();
		});
	}
	...
	_handleSubmenuKeydown(event) {
		...
		if (submenuWrapper && target.matches(SELECTOR_SUBMENU_TOGGLE) && (key === ENTER_KEY || key === SPACE_KEY || key === enterKey)) {
			event.preventDefault();
			event.stopPropagation();
			const submenu = SelectorEngine.findOne(SELECTOR_MENU, submenuWrapper);
			if (submenu) {
	...
	static clearMenus(event) {
		...
		for (const instance of Menu._openInstances) {
			...
			if (instance._menu.contains(event.target) && (event.type === "keyup" && event.key === TAB_KEY || /input|select|option|textarea|form/i.test(event.target.tagName))) continue;
	...
	static dataApiKeydownHandler(event) {
		...
		if (!getToggleButton) {
			...
			if (rootMenu !== this) getToggleButton = SelectorEngine.prev(rootMenu, SELECTOR_DATA_TOGGLE)[0] || SelectorEngine.next(rootMenu, SELECTOR_DATA_TOGGLE)[0] || SelectorEngine.findOne(SELECTOR_DATA_TOGGLE, rootMenu.parentNode);
