...
describe('Carousel', () => {
  ...
  const basicMarkup = ({ classes = 'carousel slide', autoplay = false, indicators = false } = {}) => {
    const autoplayAttr = autoplay ? ' data-cx-autoplay="true"' : ''
    const indicatorsMarkup = indicators ?
      [
        '  <div class="carousel-indicators">',
        '    <button type="button" data-cx-target="#myCarousel" data-cx-slide-to="0" class="active"></button>',
        '    <button type="button" data-cx-target="#myCarousel" data-cx-slide-to="1"></button>',
        '    <button type="button" data-cx-target="#myCarousel" data-cx-slide-to="2"></button>',
        '  </div>'
      ].join('') :
      ''
  ...
  describe('navigation', () => {
    it('should scroll the viewport to the next item and fire `slide`', () => {
      ...
      EventHandler.on(carouselEl, 'slide.cx.carousel', slideSpy)
    ...
    it('should not scroll when the `slide` event is prevented', () => {
      ...
      const carousel = new Carousel(carouselEl)
      stubLayout(carousel)
      EventHandler.on(carouselEl, 'slide.cx.carousel', event => event.preventDefault())
    ...
    it('should rest the slide at the scroll-padding (peek) offset', () => {
      ...
      const carousel = new Carousel('#myCarousel')
      stubLayout(carousel)
      // Emulate `--cx-carousel-items-peek` via the resulting scroll-padding so
  ...
  describe('seamless loop (`ends: loop`)', () => {
    it('should continue into a transient clone and teleport to the first slide when going next from the last', () => {
      ...
      EventHandler.on(carouselEl, 'slid.cx.carousel', slidSpy)
    ...
    it('should fall back to a plain wrap for multi-item layouts', () => {
      ...
      const carouselEl = fixtureEl.querySelector('#myCarousel')
      carouselEl.style.setProperty('--cx-carousel-items', '2')
      const carousel = new Carousel(carouselEl, { ends: 'loop' })
      stubLayout(carousel)
    ...
    it('should fall back to a plain wrap when a peek is configured', () => {
      fixtureEl.innerHTML = basicMarkup()
      const carouselEl = fixtureEl.querySelector('#myCarousel')
      carouselEl.style.setProperty('--cx-carousel-items-peek', '16px')
  ...
  describe('disable end controls', () => {
    const controlsMarkup = ({ slides = 3 } = {}) => {
      ...
      return [
        ...
        items,
        '  </div>',
        '  <button id="prev" class="btn-icon btn-sm" type="button" data-cx-target="#myCarousel" data-cx-slide="prev"></button>',
        '  <button id="next" class="btn-icon btn-sm" type="button" data-cx-target="#myCarousel" data-cx-slide="next"></button>',
        '</div>'
      ].join('')
    }
  ...
  describe('active sync (IntersectionObserver)', () => {
    ...
    it('should update the active indicator', () => {
      ...
      expect(active.getAttribute('data-cx-slide-to')).toEqual('2')
  ...
  describe('autoplay', () => {
    ...
    it('should respect a per-item `data-cx-interval`', () => {
      fixtureEl.innerHTML = [
        '<div id="myCarousel" class="carousel slide">',
        '  <div class="carousel-inner">',
        '    <div class="carousel-item active" data-cx-interval="2000">item 1</div>',
        '    <div class="carousel-item" data-cx-interval="4000">item 2</div>',
        '  </div>',
        '</div>'
      ].join('')
    ...
    it('should schedule the next wait from the slide being navigated to, not the current one', () => {
      ...
      fixtureEl.innerHTML = [
        '<div id="myCarousel" class="carousel slide">',
        '  <div class="carousel-inner">',
        '    <div class="carousel-item active" data-cx-interval="10000">item 1</div>',
        '    <div class="carousel-item" data-cx-interval="2000">item 2</div>',
        '  </div>',
        '</div>'
      ].join('')
    ...
    it('should toggle the playing class and expose the interval while cycling', () => {
      ...
      expect(carouselEl.style.getPropertyValue('--cx-carousel-interval')).toEqual('3000ms')
  ...
  describe('play/pause control', () => {
    const playPauseMarkup = ({ autoplay = true } = {}) => [
      `<div id="myCarousel" class="carousel slide"${autoplay ? ' data-cx-autoplay="true"' : ''}>`,
      '  <div class="carousel-inner">',
      '    <div class="carousel-item active">item 1</div>',
      '    <div class="carousel-item">item 2</div>',
      '  </div>',
      '  <button class="carousel-control-play-pause" type="button" data-cx-target="#myCarousel" data-cx-pause-label="Pause" data-cx-play-label="Play">',
  ...
  describe('data-api', () => {
    ...
    it('should go to the previous item with data-cx-slide="prev"', () => {
      fixtureEl.innerHTML = [
        '<div id="myCarousel" class="carousel slide" data-cx-ends="wrap">',
        '  <div class="carousel-inner">',
        '    <div id="item1" class="carousel-item active">item 1</div>',
        '    <div id="item2" class="carousel-item">item 2</div>',
        '  </div>',
    ...
    it('should go to a given index with data-cx-slide-to', () => {
      fixtureEl.innerHTML = basicMarkup({ indicators: true })

      const carousel = Carousel.getOrCreateInstance('#myCarousel')
      stubLayout(carousel)

      fixtureEl.querySelector('[data-cx-slide-to="2"]').click()
    ...
    it('should init carousels with data-cx-autoplay="true" on load', () => {
      fixtureEl.innerHTML = '<div id="myCarousel" data-cx-autoplay="true"><div class="carousel-inner"><div class="carousel-item active">item 1</div></div></div>'
