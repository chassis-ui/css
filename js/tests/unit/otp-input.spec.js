...
describe('OtpInput', () => {
  ...
  const getOtpHtml = (inputCount = 6, attributes = '') => {
    const inputs = Array.from({ length: inputCount })
      .map(() => '<input type="text" class="form-input">')
      .join('')
    return `<div class="form-otp" ${attributes}>${inputs}</div>`
  }
  ...
  describe('DATA_KEY', () => {
    it('should return plugin data key', () => {
      expect(OtpInput.DATA_KEY).toEqual('cx.otpInput')
    })
  })
  ...
  describe('constructor', () => {
    it('should take care of element either passed as a CSS selector or DOM element', () => {
      ...
      const otpEl = fixtureEl.querySelector('.form-otp')
      const otpBySelector = new OtpInput('.form-otp')
    ...
    it('should set input type to password when mask is true', () => {
      fixtureEl.innerHTML = getOtpHtml(6, 'data-cx-mask="true"')
  ...
  describe('events', () => {
    it('should trigger complete event when all inputs filled', () => {
      return new Promise(resolve => {
        ...
        otpEl.addEventListener('complete.cx.otp-input', event => {
    ...
    it('should trigger input event on each input change', () => {
      return new Promise(resolve => {
        ...
        otpEl.addEventListener('input.cx.otp-input', event => {
