...
- **Tailwind `dark:`/`light:` variants:** an element inside a same-value nested `data-cx-theme` (a dark navbar inside a dark page) no longer loses the variant when the OS color scheme is the opposite; only an opposite-value attribute counts as a nearer override
- **`chassisPrefix({ tailwind: true })`:** every key declared in an `@theme` block, and `var()` references to it, now stays unprefixed. Previously only `--breakpoint-*`/`--container-*` did, so a project's own `@theme { --font-sans: …; }` became `--cx-font-sans` and Tailwind ignored it
- **`--chassis-prefix` marker:** no longer lands in a `:root` rule nested in `@media` (`dist/tailwind/reboot.css` put it in the `prefers-reduced-motion` block, so visitors with reduced motion got no marker). A prefixed stylesheet without an unconditional `:root` rule, such as a component-only build, now gets one for the marker instead of none
