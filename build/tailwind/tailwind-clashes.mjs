...
// \`$breakpoints\`.
//
// Regenerated by \`build/tailwind/tailwind-clashes.mjs\` (checked on every
...
export async function coreUtilityAfterReset(candidate, themeCss) {
  const css = [TAILWIND_UTILITIES_PROBE, themeCss, '@source not "..";'].join('\n')
...
async function mergedUtility(candidate, themeCss, utilitiesCssText) {
  const css = [TAILWIND_UTILITIES_PROBE, themeCss, utilitiesCssText, '@source not "..";'].join('\n')
  const compiler = await compile(css, { base: outDir, onDependency: () => {} })
...
export async function checkUtilityNameClashes() {
  ...
  for (const [name, { classification }] of live) {
    if (!knownNames.has(name)) {
      drift.push(
        `new clash "${name}" (${classification}) is not in build/tailwind/tailwind-utility-clashes.json`
      )
      continue
    }
  ...
  for (const name of knownNames) {
    if (!live.has(name))
      drift.push(`"${name}" is in the policy file but is no longer a same-name clash at all`)
  }
  if (drift.length > 0) {
    throw new Error(
      `tailwind-clashes: utility-name clash set has drifted from build/tailwind/tailwind-utility-clashes.json ` +
...
// \`build/tailwind/tailwind-utility-clashes.json\` (clashes with Tailwind core) and
// \`build/tailwind/tailwind-bridge-clashes.json\` (clashes the opt-in token bridge
