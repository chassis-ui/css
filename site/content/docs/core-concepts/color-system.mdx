...
<div class="overflow-scroll text-nowrap mb-md">
  <div class="grid font-code" style="--cx-grid-columns: 11; --cx-grid-gap: var(--cx-space-2xs);">
  {
    getData('colors').map((color) => {
      return (
        <div class="grid" style="width: 12rem; --cx-grid-columns: 1; grid-template-rows: auto;">
          <div class={`p-md rounded-sm mb-xs position-relative swatch-${color.name} bg-${color.name} fg-contrast`}>
            <strong class="d-block">{color.name}</strong>
            {sassVars('color')?.[color.name]}
          </div>
          <div class={`p-md rounded-sm cxd-${color.name}-05`}>{color.name}-05</div>
            {
              Array.from({ length: 9 }, (_, i) => {
                const value = (i + 1) * 10; // Generates 10, 20, ..., 90
                return <div class={`p-md rounded-sm cxd-${color.name}-${value}`}>{color.name}-{value}</div>
              })
            }
          <div class={`p-md rounded-sm cxd-${color.name}-95`}>{color.name}-95</div>
        </div>
      )
    })
  }
...
<div class="row g-xs mb-md font-code">
  {
    getData('context-colors').map((color) => {
      return (
        <div class="md:col-4">
          <div class={`p-md m-0 bg-${color.name} fg-contrast rounded-sm`}>--cx-{color.name}</div>
        </div>
      )
    })
  }
