...
The Chassis CSS Datepicker is a wrapper around [Vanilla Calendar Pro](https://vanilla-calendar.pro/) that provides a consistent, accessible date selection experience. It supports light/dark themes, input binding, and flexible configuration via data attributes or JavaScript.
...
- The picker respects Chassis CSS’s color modes (`data-cx-theme`)
...
Use the [form adorn component]([[docsref:/forms/input-adorn]]) to add a calendar icon alongside the datepicker input. When the input is inside a `.form-adorn` wrapper, the calendar automatically positions relative to the wrapper instead of the input.
...
The datepicker automatically adapts to Chassis CSS’s color modes. When `data-cx-theme="dark"` is set on a parent element or the `<html>` tag, the calendar popup inherits that theme.
...
For features not directly exposed by Chassis CSS’s options, use `vcpOptions` to pass any Vanilla Calendar Pro setting:
