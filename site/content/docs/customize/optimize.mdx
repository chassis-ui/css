...
This applies to [the Tailwind entry point]([[docsref:/getting-started/tailwind]]) too, and matters more there: Tailwind's own build only tree-shakes the `@utility` rules it generates from classes it finds in your markup, not the plain component CSS Chassis ships in the `components` layer. An unused component still needs to be trimmed at the Sass level, the same way as shown here.

### Trim unused breakpoints

Chassis CSS generates responsive surfaces — utility prefixes (`sm:d-flex`, `lg:m-md`), grid columns (`.col-lg-6`), container query loops, and the `--breakpoint-*` CSS custom properties — from a single `$breakpoints` map. Projects that don't need every breakpoint can shrink the compiled CSS substantially by overriding the map before importing Chassis:
...
- The `xs` breakpoint must remain. It carries the unprefixed mobile-first base classes that every responsive utility builds on.
- Several Chassis components hardcode `sm`, `md`, and `lg` in their internal responsive logic. Removing any of those three breaks those components. The `xl` and `2xl` breakpoints are not hardcoded anywhere internal and are safe to remove.
...
- **Responsive prefix classes (`sm:d-flex`, `lg:m-md`) contain a colon.** The default extractor strips everything after a colon, treating these as pseudo-selectors. The custom `defaultExtractor` above (`[\w/:-]+(?<!:)`) preserves internal colons while still rejecting trailing ones, so responsive utilities survive the purge.
