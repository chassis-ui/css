...
| `$enable-responsive-utilities`    | `true` (default) or `false`        | Enables [Breakpoint Type Scale]([[docsref:/content/typography#breakpoint-type-scale]]) utility classes for responsive typography based on breakpoints. |
| `$enable-rfs`                     | `true` (default) or `false`        | Enables [Responsive Font Size]([[docsref:/getting-started/rfs]]) for automatic font size scaling based on viewport. Provides better typography across device sizes. |
