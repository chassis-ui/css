...
| `$enable-accordion-sizes`         | `true` (default) or `false`        | Generates size variations for accordions (small, medium, large), allowing for different visual weights in layouts. |
...
| `$enable-caret`                   | `true` (default) or `false`        | Shows a directional indicator (caret) on `.menu-toggle` elements, improving affordance for interactive dropdown elements. |
...
| `$enable-negative-margins`        | `true` (default) or `false`        | Generates [negative margin utilities]([[docsref:/utilities/margin#negative-margins]]) like `.-m-medium`, `-mt-small`. Useful for precise layout adjustments. |
