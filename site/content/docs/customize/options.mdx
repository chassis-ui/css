...
| `$enable-caret`                   | `true` (default) or `false`        | Adds a caret automatically to native `select` elements, comboboxes, and menu toggles. |
| `$enable-background-gradients`    | `true` (default) or `false`        | Applies subtle gradient backgrounds to various components via `background-image`. Disable for a flatter visual style or to reduce CSS size. |
| `$enable-component-shadows`       | `true` (default) or `false`        | Enables predefined decorative `box-shadow` styles on various components for depth. |
...
| `$enable-cssgrid`                 | `true` (default) or `false`        | Enables the modern CSS Grid system with classes like `.grid` and `.md:g-col-1`. Provides two-dimensional layout capabilities. |
...
| `$enable-grid-classes`            | `true` (default) or `false`        | Generates classes for the flexbox-based grid system (`.row`, `.md:col-1`, etc.). Consider disabling if you only use CSS Grid. |
...
| `$enable-negative-margins`        | `true` (default) or `false`        | Generates [negative margin utilities]([[docsref:/utilities/margin#negative-margins]]) like `.-m-md`, `-mt-sm`. Useful for precise layout adjustments. |
| `$enable-reduced-motion`          | `true` (default) or `false`        | Respects the [`prefers-reduced-motion` media query]([[docsref:/core-concepts/accessibility#reduced-motion]]), reducing or eliminating animations for users who prefer minimal motion. Critical for vestibular disorders. |
| `$enable-responsive-font-sizes`     | `false` (default) or `true`        | Enables [responsive size]([[docsref:/content/typography#responsive-sizes]]) utility classes for locking font sizes to specific breakpoints. |
| `$enable-fluid-font-sizes`          | `true` (default) or `false`        | Enables [fluid sizing]([[docsref:/content/typography#fluid-sizes]]) — font sizes scale smoothly between a minimum and maximum using `clamp()`. |
...
$enable-background-gradients:    false !default;
...
$enable-fluid-font-sizes:                    true !default;
...
- **Visual Enhancements**: Disable `$enable-background-gradients` and `$enable-component-shadows` for a more minimal style
...
- **Simplify Rendering**: Disable `$enable-component-shadows` and `$enable-background-gradients` to reduce GPU/CPU load
- **Optimize Typography**: Consider `$enable-fluid-font-sizes: false` if you don't need responsive font scaling
