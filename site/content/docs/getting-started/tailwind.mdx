...
The preset and `@tailwindcss/postcss` can go in either order in the `plugins` array — verified to produce byte-identical output either way.
...
The utility-name clashes documented above are checked against Chassis's own `$utilities` map at build time; the reviewed, versioned result lives in [`build/tailwind/tailwind-utility-clashes.json`]([[config:repo]]/blob/main/build/tailwind/tailwind-utility-clashes.json) and [`build/tailwind/tailwind-bridge-clashes.json`]([[config:repo]]/blob/main/build/tailwind/tailwind-bridge-clashes.json). That check doesn't re-run against a project's own customized `$utilities` — a renamed class, a new entry, or an [Options]([[docsref:/customize/options]]) override can introduce a same-name clash of its own, against Tailwind core or against Chassis's other generated names, that nothing here catches automatically. Those two JSON files are the reference for what's already known to clash and how each case was resolved — useful for triaging a new one by hand.
