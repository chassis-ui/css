---
title: Parcel
description: Bundle Chassis CSS and JavaScript with Parcel — zero-config Sass detection and token resolution.
toc: true
---

[Parcel](https://parceljs.org/) is a zero-configuration web application bundler that automatically installs language transformers as it detects them. This guide walks through a fresh project; if you already have Parcel set up, jump to [Import Chassis](#import-chassis).

[Installation]([[docsref:/getting-started/installation]]) covers the moving parts — this is the lightest of the three bundler guides because Parcel's auto-detection eliminates most manual configuration.

## Setup

This guide assumes Node.js is installed and you're comfortable in a terminal.

1. **Create a project folder and initialize npm.**

   ```bash
   mkdir my-project && cd my-project
   pnpm init
   ```

2. **Install Parcel.** Parcel is the only build dependency — it installs the Sass transformer automatically when it sees a `.scss` file.

   ```bash
   pnpm add -D parcel
   ```

3. **Install Chassis CSS and its tokens.** Add the optional peer dependencies for the components that need them, or omit both and use `chassis.bundle.*` which includes them.

   ```bash
   pnpm add @chassis-ui/css @chassis-ui/tokens
   pnpm add @floating-ui/dom vanilla-calendar-pro   # optional
   ```

## Project structure

Create the source folders and starter files:

```bash
mkdir -p src/js src/scss
touch src/index.html src/js/main.js src/scss/styles.scss
```

The result:

```text
my-project/
├── src/
│   ├── js/
│   │   └── main.js
│   ├── scss/
│   │   └── styles.scss
│   └── index.html
├── package.json
└── pnpm-lock.yaml
```

## Configure Parcel

Parcel itself needs no configuration file. The HTML page is the entry point, and Parcel follows the imports from there.

1. **Fill in `src/index.html`.** Reference the stylesheet and the script directly — Parcel resolves both, compiles them, and rewrites the references at build time.

   ```html
   <!doctype html>
   <html lang="en">
     <head>
       <meta charset="utf-8">
       <meta name="viewport" content="width=device-width, initial-scale=1">
       <title>Chassis CSS w/ Parcel</title>
       <link rel="stylesheet" href="scss/styles.scss">
       <script type="module" src="js/main.js"></script>
     </head>
     <body>
       <div class="container py-lg px-md mx-auto">
         <h1>Hello, Chassis CSS and Parcel!</h1>
         <button class="button primary">Primary button</button>
       </div>
     </body>
   </html>
   ```

   When Parcel sees the `.scss` reference, it auto-installs the [Sass transformer](https://parceljs.org/languages/sass/). To install it manually instead, run `pnpm add -D @parcel/transformer-sass`.

2. **Add the `start` and `build` scripts to `package.json`.**

   ```json
   {
     "scripts": {
       "start": "parcel serve src/index.html --public-url / --dist-dir dist",
       "build": "parcel build src/index.html --dist-dir dist"
     }
   }
   ```

3. **Start the dev server.**

   ```bash
   pnpm start
   ```

   Parcel opens a local server on `http://localhost:1234`. The page won't be styled yet — the stylesheet and script still need to be filled in.

## Import Chassis

Parcel needs a Sass entry point and a `.sassrc.js` config file that sets `loadPaths` so Sass can resolve token overrides from `src/scss`, plus the framework's own default tokens and bare package specifiers under `node_modules` — unlike Vite, Webpack, and Astro, Parcel doesn't resolve `node_modules` for Sass automatically.

1. **Create `.sassrc.js` in the project root.** Parcel passes these options directly to Dart Sass.

   ```js
   const path = require('path')

   module.exports = {
     loadPaths: [
       path.resolve(__dirname, 'src/scss'),                                    // project token override
       path.resolve(__dirname, 'node_modules/@chassis-ui/css/scss/vendor'),    // framework's default tokens
       path.resolve(__dirname, 'node_modules')                                 // bare package specifiers (@chassis-ui/tokens/…)
     ]
   }
   ```

2. **Fill in `src/scss/styles.scss`.** The token source resolves through `_vendor.scss` before the rest of Chassis processes its variables, so a single `@use` directive is all that's needed.

   ```scss
   // Chassis CSS — token source resolves via the loadPaths entry in .sassrc.js
   @use "@chassis-ui/css/scss/chassis";

   // Optional: settings overrides use the `with (...)` syntax
   // @use "@chassis-ui/css/scss/chassis" with (
   //   $enable-fluid-font-sizes: false,
   //   $enable-component-shadows: true,
   // );
   ```

   The default token build (`chassis/docs`) is resolved automatically. To switch to a custom token set, place a `_chassis-tokens.scss` on an earlier `loadPaths` entry — see [Customize → Sass → Token sources]([[docsref:/customize/sass#token-sources]]) for details.

3. **Import the Chassis JS in `src/js/main.js`.** Floating UI and vanilla-calendar-pro are resolved through the bundler when needed.

   ```js
   import * as chassis from '@chassis-ui/css'
   ```

   For smaller bundles, import only the required plugins:

   ```js
   import { Modal, Tooltip } from '@chassis-ui/css'
   ```

4. **Reload.** With all files filled in, Parcel hot-reloads the page — the heading and button should now render with Chassis styling.

## Apply the `--cx-` prefix

Chassis's own components and JS plugins read some CSS custom properties with a `--cx-` prefix (`--cx-carousel-interval`, `--cx-breakpoint-*`) — Sass only ever writes the unprefixed name (`--carousel-interval`), so a separate PostCSS step adds the prefix. Parcel auto-detects PostCSS config the same way it auto-detects the Sass transformer:

```bash
pnpm add -D postcss-prefix-custom-properties
```

```js
// postcss.config.mjs
import { chassisPrefix } from '@chassis-ui/css/postcss'

export default {
  plugins: [chassisPrefix()]
}
```

Reload the page; `getComputedStyle` on any Chassis component now shows its `--cx-`-prefixed properties.

## Build optimizations

Parcel automatically extracts CSS to a separate file when running `parcel build`. No additional configuration is needed for CSS extraction.

## Next steps

These docs cover JavaScript integration, Sass customization, and bundle optimization for a Parcel-based Chassis project.

- [JavaScript]([[docsref:/getting-started/javascript]]) — using the JS plugins, the programmatic API, and event names.
- [Customize → Sass]([[docsref:/customize/sass]]) — variable overrides and importing only the required partials.
- [Customize → Optimize]([[docsref:/customize/optimize]]) — keeping the Chassis output lean.
