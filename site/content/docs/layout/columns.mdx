...
Use `.order-` classes for controlling the **visual order** of your content. These classes are responsive, so you can set the `order` by breakpoint (e.g., `.order-1.medium:order-2`). Includes support for `1` through `5` across all six grid tiers.
...
Move columns to the right using `.offset-medium-*` classes. These classes increase the left margin of a column by `*` columns. For example, `.medium:offset-4` moves `.medium:col-4` over four columns.
...
<Example code={`<div class="clearfix">
    <Placeholder width="100%" height="210" class="medium:col-6 medium:float-end mb-medium medium:ms-3" text="Responsive floated image" />
