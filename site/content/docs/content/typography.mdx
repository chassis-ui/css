...
### Breakpoint Type Scale

Chassis - CSS introduces Breakpoint Type Scale utilities that generate responsive font sizes based on defined breakpoints. Enable this feature through the `$enable-bts` variable in your customization options. This allows for granular control over typography across different device sizes, ensuring optimal readability and design consistency.

The Breakpoint Type Scale follows a mobile-first approach, allowing you to stack responsive font size classes. Classes are named using the format `font-{size}` for the base size (applies to all viewports from `xsmall` and up) and `font-{breakpoint}-{size}` for breakpoint-specific sizes (`small`, `medium`, `large`, `xlarge`, and `2xlarge`). Each breakpoint class overrides the previous font size at that breakpoint and larger viewports.

<Example code={`<p class="font-small font-medium-large font-large-2xlarge font-xlarge-4xlarge">
