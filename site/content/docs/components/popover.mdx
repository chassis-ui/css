...
| `placement` | string, function | `'right'` | Preferred placement. Logical values `start` and `end` adapt to text direction. Physical values `top`, `bottom`, `left`, and `right` always place the popover on that side. Accepts responsive prefixes (`'top md:right'`). A function receives the popover element and the trigger element and must return a placement string. |
