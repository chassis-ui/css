...
Add contextual information to slides with the `.carousel-caption` element. Chassis CSS captions integrate seamlessly with the utility system for responsive behavior. Use [display utilities]([[docsref:/utilities/display]]) to control caption visibility across viewport sizes—this example uses `.d-none` for mobile and `.medium:d-block` for larger screens.
