...
- Wrap the `.nav` in a `.nav-overflow` element and add `data-cx-toggle="nav-overflow"` to that wrapper.
...
- **Responds to container size**, not viewport size. The component watches the wrapper with a ResizeObserver, so it works perfectly in embedded contexts, documentation examples, and responsive containers.
...
Wrap the nav in `<div class="nav-overflow" data-cx-toggle="nav-overflow">`. When items don't fit, they'll automatically move to a "More" menu. Drag the right edge of the container below to see how nav items automatically move to the "More" menu as space becomes limited.

<ResizableExample code={`<div class="nav-overflow" data-cx-toggle="nav-overflow">
...
<ResizableExample code={`<nav class="navbar navbar-expand bg-body-tertiary" aria-label="Main navigation">
...
```js
const nav = document.querySelector('.nav-overflow')
...
new chassis.NavOverflow(nav, {
  ...
  moreText: false // Icon-only toggle; falls back to an aria-label of "More"
})
```
...
Use the `collapseBelow` option to force all items into the overflow dropdown when the wrapper's width is below a threshold. Pass a breakpoint name to resolve the value from `--cx-breakpoint-{name}`, or a number for a direct pixel value.

```html
<div class="nav-overflow" data-cx-toggle="nav-overflow" data-cx-collapse-below="medium">
  <ul class="nav nav-pills">
    <li class="nav-item"><a class="nav-link" href="#">Link 1</a></li>
    <li class="nav-item"><a class="nav-link" href="#">Link 2</a></li>
...
Wrap the `.nav` in a `.nav-overflow` element and add `data-cx-toggle="nav-overflow"` to that wrapper to automatically enable the overflow behavior.

<CxTable>
| Attribute | Description |
| --- | --- |
| `data-cx-toggle="nav-overflow"` | Enables responsive overflow handling on the `.nav-overflow` wrapper. |
</CxTable>
...
Initialize the nav overflow component manually on the wrapper:

```js
const wrapperElement = document.querySelector('.nav-overflow')
const navOverflow = new chassis.NavOverflow(wrapperElement, {
  moreText: 'More',
  threshold: 2
})
```
...
| `moreText` | string\|boolean | `'More'` | Text label for the overflow toggle button. Pass `false` for an icon-only toggle; the button falls back to `aria-label="More"` so it keeps an accessible name. |
...
| --- | --- |
| `update.cx.navoverflow` | Fired when the overflow calculation is updated (on resize or manual update). |
| `overflow.cx.navoverflow` | Fired when items are moved to the overflow menu. Event includes `overflowCount` and `visibleCount` properties. |
</CxTable>

```js
const myNav = document.getElementById('myNav')

myNav.addEventListener('overflow.cx.navoverflow', event => {
