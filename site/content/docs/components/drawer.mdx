...
The `.drawer-footer` uses `flex-direction: row-reverse`, so buttons are right-aligned without `justify-content` — write the **primary action first** in source, it renders on the right. At the sm breakpoint, buttons stretch to equal widths (`flex: 1`). Add `.stacked` to stack them as full-width columns instead.
...
| `.max-sm:drawer` | Drawer below `sm`, inline above |
| `.max-md:drawer` | Drawer below `md`, inline above |
| `.max-lg:drawer` | Drawer below `lg`, inline above |
| `.max-xl:drawer` | Drawer below `xl`, inline above |
| `.max-2xl:drawer` | Drawer below `2xl`, inline above |
</CxTable>
...
<Example code={`<button class="button primary lg:d-none" type="button" data-cx-toggle="drawer" data-cx-target="#responsiveDrawer">Open drawer</button>

<dialog class="max-lg:drawer drawer-end" id="responsiveDrawer" aria-labelledby="responsiveDrawerLabel">
  <div class="drawer-header">
    <h2 class="drawer-title" id="responsiveDrawerLabel">Navigation</h2>
    <button type="button" class="close-button" data-cx-dismiss="drawer" aria-label="Close"></button>
  </div>
  <div class="drawer-body">
    <p>This is a <code>.max-lg:drawer</code> — a drawer on small screens, inline content on large screens.</p>
  </div>
</dialog>`} />
