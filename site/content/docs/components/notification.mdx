...
<Example class="vstack gap-md" code={getData('context-colors').map((contextColor) => `<div class="notification ${contextColor.name}" role="status">
...
<Example class="vstack gap-md" code={getData('context-colors').map((contextColor) => `<div class="notification ${contextColor.name} solid" role="status">
    <p>A ${contextColor.name} notification with an <a href="#">example link</a>.</p>
  </div>`)} />
...
<Example addStackblitzJs code={`<div id="notificationStack" role="region" aria-label="Notifications" class="vstack gap-sm mb-md"></div>
...
<Example class="vstack gap-md" code={`<div class="notification primary fade show" role="status">
...
<Callout type="info" title="Close-button spacing">
  When `.close-button` sits as a direct child of `.notification`, a `:has()` rule extends the inline-end padding so the message text doesn't run under the button. Wrapping the button breaks the selector — reserve the space manually with a utility like `pe-md`.
</Callout>
...
<Example class="vstack gap-md" code={`<div class="notification info" role="status">
...
<Example class="vstack gap-md" code={`<div class="notification success fade show" role="status">
...
When a notification carries action buttons, use flex utilities to compose them with the message and adapt across breakpoints. The example below stacks the message above the actions on narrow viewports and lays them side-by-side from `xl` upward.

<Example class="vstack gap-md" code={`<div class="notification danger fade show" role="status">
    <Icon name="exclamation-triangle-solid" class="notification-icon align-self-start xl:align-self-center" />
    <div class="d-flex flex-column xl:flex-row gap-md">
      <p class="m-0">A dismissible notification with inline actions — stacked on narrow viewports, side-by-side from xl up.</p>
      <div class="hstack gap-sm align-items-center justify-content-end">
        <button type="button" class="button danger outline sm" data-cx-dismiss="notification" aria-label="Close notification">Dismiss</button>
        <button type="button" class="button danger sm">Take Action</button>
      </div>
    </div>
  </div>`} />
