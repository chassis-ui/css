...
<Example code={`<div class="progress" role="progressbar" aria-label="Example with label" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">
    <div class="progress-bar text-end" style="width: 25%"><span class="mx-2xsmall">25%</span></div>
  </div>`} />
...
When using custom background colors with labels, ensure sufficient contrast by applying [contrast utilities]([[docsref:/helpers/contrast]]) for optimal readability.
...
<ScssDocs name="progress-variables" file="scss/config/_defaults.scss" />
