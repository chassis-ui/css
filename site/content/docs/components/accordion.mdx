---
title: Accordion
description: Create accessible, responsive accordions using HTML5 native elements with Chassis CSS. Features smooth transitions, multiple styling options, and built-in accessibility without extensive JavaScript.
aliases:
 - "/components"
 - "[[docsref:/components]]"
tokens: component
js: optional
css_layer: components
toc: true
---

## How It Works

Chassis accordions leverage the native HTML5 `<details>` and `<summary>` elements, providing several key benefits:

- **Native browser functionality**: The browser handles expanding and collapsing content without requiring extensive JavaScript
- **Built-in accessibility**: Native keyboard navigation and screen reader support through the HTML5 elements
- **Styled with CSS**: Apply the `.accordion` class to the wrapper for visual styling of `<details>` elements.
- **Enhanced with JavaScript**: Add the `data-cx-accordion` attribute to the wrapper to opt in smooth transitions via JS
- **Group behavior**: Using the same `name` attribute on multiple `<details>` elements creates accordion groups where only one item can be open at a time

<Callout name="info-prefersreducedmotion" />

## Examples

Click the accordions below to expand/collapse the content. Each item in this example shares the same `name="example"` attribute, creating a group where only one accordion can be open at a time.

Add the `open` attribute to any `<details>` element to render it expanded by default.

<Example code={`<div class="accordion" data-cx-accordion>
  <details name="example" open>
    <summary>
      <span class="accordion-title">Accordion Item #1</span>
    </summary>
    <div class="accordion-body">
      <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
    </div>
  </details>
  <details name="example">
    <summary>
      <span class="accordion-title">Accordion Item #2</span>
    </summary>
    <div class="accordion-body">
      <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
    </div>
  </details>
  <details name="example">
    <summary>
      <span class="accordion-title">Accordion Item #3</span>
    </summary>
    <div class="accordion-body">
      <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
    </div>
  </details>
</div>`} />

### Headings

The HTML spec permits one heading element as a direct child of `<summary>` — no ARIA role overrides are needed. Use a heading element as `.accordion-title` when accordion headers need to participate in the document's visual and source-order hierarchy.

<Example code={`<div class="accordion" data-cx-accordion>
  <details name="example-headings" open>
    <summary>
      <h3 class="accordion-title">Accordion Item #1</h3>
    </summary>
    <div class="accordion-body">
      <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
    </div>
  </details>
  <details name="example-headings">
    <summary>
      <h3 class="accordion-title">Accordion Item #2</h3>
    </summary>
    <div class="accordion-body">
      <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
    </div>
  </details>
  <details name="example-headings">
    <summary>
      <h3 class="accordion-title">Accordion Item #3</h3>
    </summary>
    <div class="accordion-body">
      <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
    </div>
  </details>
</div>`} />

The heading level does not appear in assistive technology heading navigation — inside a disclosure button, heading semantics are absorbed into the button role. Accordion headers remain fully accessible via keyboard and button navigation. When heading-outline navigation is a strict requirement, use the [WAI-ARIA APG accordion pattern](https://www.w3.org/WAI/ARIA/apg/patterns/accordion/) instead.

### Static Items

Accordions with the same `name` attribute form groups that close automatically when another item in the group is opened. Items without a name attribute (or with unique names) remain independent. You can mix both types in a single accordion.

<Example code={`<div class="accordion" data-cx-accordion>
    <details open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the first item's body, shown by default. Since it has <b>no name attribute</b>, it behaves independently and will only close when its own summary element is clicked.</p>
      </div>
    </details>
    <details name="example-static" open>
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This item shares the <b>example-static</b> name attribute with Accordion Item #3, forming a group. It will automatically close when <b>Accordion Item #3</b> is opened.</p>
      </div>
    </details>
    <details name="example-static">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This item shares the <b>example-static</b> name attribute with Accordion Item #2, forming a group. It will automatically close when <b>Accordion Item #2</b> is opened.</p>
      </div>
    </details>
  </div>`} />

### Flush Accordion

Add `.flush` to remove some borders and rounded corners to render accordions edge-to-edge with their parent container.

<Example code={`<div class="accordion flush" data-cx-accordion>
    <details name="example-flush" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-flush">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-flush">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>`}
  customMarkup={`<div class="accordion flush" data-cx-accordion>
    ...
  </div>`} />


### Caret Position

Add `.caret-end` to the accordion container to move the caret icon to the end of the summary element.

<Example code={`<div class="accordion caret-end" data-cx-accordion>
    <details name="example-end" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-end">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-end">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>`}
  customMarkup={`<div class="accordion caret-end" data-cx-accordion>
    ...
  </div>`} />

### Without Animation

Without the `data-cx-accordion` attribute, the accordion will function without smooth transitions. The content will open and close instantly and the caret icon will not animate.

<Example code={`<div class="accordion">
    <details name="example-no-transition" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-no-transition">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-no-transition">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>`}
  customMarkup={`<div class="accordion">
    ...
  </div>`} />


## Sizes

<InFigma plural={true} />

Chassis CSS offers size variations to help accordions integrate seamlessly with different UI contexts. Adjust sizes to establish visual hierarchy and optimize space utilization based on your layout requirements.

### Large

Add `.lg` for larger font size and more generous spacing. Suitable for primary content areas, FAQ sections, or hero elements where accordions need more visual prominence.

<Example code={`<div class="accordion lg" data-cx-accordion>
    <details name="example-lg" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-lg">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-lg">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>`}
  customMarkup={`<div class="accordion lg" data-cx-accordion>
    ...
  </div>`} />

### Small

Add `.sm` for reduced font size and compact spacing. Suitable for secondary content areas like cards, sidebars, or dense layouts where space efficiency is a priority.

<Example code={`<div class="accordion sm" data-cx-accordion>
    <details name="example-sm" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-sm">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-sm">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>`}
  customMarkup={`<div class="accordion sm" data-cx-accordion>
    ...
  </div>`} />

## Context Variants

<CSSOnly />

You can use the `.context` class to add a color or style to the accordion. The context class is used to apply a specific color scheme or style to the accordion, making it visually distinct and aligned with your design preferences. See [context class]([[docsref:core-concepts/context-class]]) for more information.

### Colors

Use `.context` class on accordion with a color, like `.primary`.

<Example code={`<div class="row row-cols-1 md:row-cols-2 g-lg">

<div class="col">
  <div class="accordion context neutral sm" data-cx-accordion>
    <details name="example-neutral" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-neutral">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-neutral">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>
</div>

<div class="col">
  <div class="accordion context primary sm" data-cx-accordion>
    <details name="example-primary" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-primary">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-primary">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>
</div>

<div class="col">
  <div class="accordion context secondary sm" data-cx-accordion>
    <details name="example-secondary" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-secondary">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-secondary">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>
</div>

<div class="col">
  <div class="accordion context alternate sm" data-cx-accordion>
    <details name="example-alternate" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-alternate">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-alternate">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>
</div>

  </div>`}
  customMarkup={`<div class="accordion context neutral sm" data-cx-accordion>
    ...
  </div>

  <div class="accordion context primary sm" data-cx-accordion>
    ...
  </div>

  <div class="accordion context secondary sm" data-cx-accordion>
    ...
  </div>

  <div class="accordion context alternate sm" data-cx-accordion>
    ...
  </div>`} />


### Styles

Use `.context` class on accordions with a color and style, like `.primary` and `.solid`.

<Example code={`<div class="row row-cols-1 md:row-cols-2 g-lg">

<div class="col">
  <div class="accordion context primary sm" data-cx-accordion>
    <details name="example-basic" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-basic">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-basic">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>
</div>

<div class="col">
  <div class="accordion context primary solid sm" data-cx-accordion>
    <details name="example-solid" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-solid">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-solid">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>
</div>

<div class="col">
  <div class="accordion context primary smooth sm" data-cx-accordion>
    <details name="example-smooth" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-smooth">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-smooth">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>
</div>

<div class="col">
  <div class="accordion context primary outline sm" data-cx-accordion>
    <details name="example-outline" open>
      <summary>
        <span class="accordion-title">Accordion Item #1</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>first item's body</b>, shown by default. It will automatically close when another item is opened by clicking its summary element.</p>
      </div>
    </details>
    <details name="example-outline">
      <summary>
        <span class="accordion-title">Accordion Item #2</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>second item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
    <details name="example-outline">
      <summary>
        <span class="accordion-title">Accordion Item #3</span>
      </summary>
      <div class="accordion-body">
        <p>This is the <b>third item's body</b>. It is hidden by default and will open when its summary element is clicked.</p>
      </div>
    </details>
  </div>
</div>

  </div>`}
  customMarkup={`<div class="accordion context primary sm" data-cx-accordion>
    ...
  </div>

  <div class="accordion context primary solid sm" data-cx-accordion>
    ...
  </div>

  <div class="accordion context primary smooth sm" data-cx-accordion>
    ...
  </div>

  <div class="accordion context primary outline sm" data-cx-accordion>
    ...
  </div>`} />

## Accessibility

The accordion component leverages the native HTML5 `<details>` and `<summary>` elements, which provide built-in accessibility features including keyboard navigation, focus management, and appropriate ARIA states without additional JavaScript. This ensures users of assistive technologies can interact with accordions using standard navigation patterns.

### Using Headings

The HTML spec permits one heading element as a direct child of `<summary>`. No ARIA role overrides are needed — place the heading inside `<summary>` and the browser handles button semantics and `aria-expanded` state automatically.

```html
<details>
  <summary>
    <h3 class="accordion-title">Accordion Item #1</h3>
  </summary>
  <div class="accordion-body">
    <!-- Content -->
  </div>
</details>
```

Screen readers announce the header as a disclosure button using the heading text as its accessible name. The `<summary>` retains its native `role="button"` and the browser reflects the `open` attribute as `aria-expanded` without JavaScript.

The heading level does not appear in assistive technology heading-outline navigation. Inside a disclosure button, heading semantics are absorbed into the button role — the heading element contributes its text as the button's accessible name, but its level is not exposed in the document outline. Accordion headers are discoverable via button and form-control navigation but not via heading shortcuts (h-key in NVDA, JAWS, or VoiceOver).

When AT heading-outline navigation is a hard requirement, the [WAI-ARIA APG accordion pattern](https://www.w3.org/WAI/ARIA/apg/patterns/accordion/) uses `<h3><button>` markup that participates in both heading and button navigation — at the cost of abandoning the native `<details>` element and requiring full JavaScript state management.

## CSS

<DocsCSS exposed />

### Custom properties

<DocsCSSVars name="Accordion" cascading />

<ScssDocs name="accordion-css-vars" file="scss/_accordion.scss" compile />

### Sass variables

<DocsSASSVars name="Accordion" exposed />

<ScssDocs name="accordion-variables" file="scss/config/_defaults.scss" />

### Design tokens

<DocsDesignTokens name="Accordion" />

<ScssDocs name="accordion-tokens" file="scss/tokens/_accordion.scss" />
