...
var Menu = class Menu extends FloatingBase {
	...
	_getConfig(config) {
		...
		if (typeof config.reference === "object" && !isElement(config.reference) && typeof config.reference.getBoundingClientRect !== "function") throw new TypeError(`${NAME$16.toUpperCase()}: Option "reference" provided type "object" without a required "getBoundingClientRect" method.`);
		return config;
	}
	_createFloating() {
...
var Drawer = class extends DialogBase {
	...
	_initSwipe() {
		...
		else if (element.classList.contains("drawer-end")) swipeConfig[isRTL() ? "leftCallback" : "rightCallback"] = () => this.hide();
		else swipeConfig[isRTL() ? "rightCallback" : "leftCallback"] = () => this.hide();
