/*!
* Chassis v0.4.0 (https://chassis-ui.com)
...
var Config = class {
	...
	_mergeConfigObj(config, element) {
		...
		const dataAttributes = isElement$1(element) ? Manipulator.getDataAttributes(element) : {};
...
* Constants
*/
const NAME$22 = "accordion";
const EVENT_KEY$19 = `.cx.accordion`;
const DATA_API_KEY$13 = ".data-api";
const EVENT_OPEN = `open${EVENT_KEY$19}`;
const EVENT_OPENED = `opened${EVENT_KEY$19}`;
const EVENT_CLOSE$2 = `close${EVENT_KEY$19}`;
const EVENT_CLOSED$1 = `closed${EVENT_KEY$19}`;
const ATTR_CLONE = "data-cx-clone";
const SELECTOR_DETAILS = `[data-cx-accordion] > details:not([${ATTR_CLONE}])`;
const SELECTOR_SUMMARY = "summary";
const SELECTOR_CONTENT$1 = ".accordion-body";
const EVENT_CLICK_DATA_API$10 = `click${EVENT_KEY$19}${DATA_API_KEY$13}`;
/**
* Class definition
*/
var Accordion = class extends BaseComponent {
	...
	toggle() {
		if (this._element.open) this.close();
		else this.open();
	}
	open() {
	...
	_createObserver() {
		return new MutationObserver((mutationsList) => {
			for (const mutation of mutationsList) if (mutation.attributeName === "open") {
				if (this._element.open) this.open();
				else this.close();
				break;
			}
		});
...
* Constants
*/
const NAME$21 = "button";
const EVENT_KEY$18 = `.cx.button`;
const DATA_API_KEY$12 = ".data-api";
const CLASS_NAME_ACTIVE$5 = "active";
const SELECTOR_DATA_TOGGLE$11 = "[data-cx-toggle=\"button\"]";
const EVENT_CLICK_DATA_API$9 = `click${EVENT_KEY$18}${DATA_API_KEY$12}`;
/**
* Class definition
*/
...
* Data API implementation
*/
EventHandler.on(document, EVENT_CLICK_DATA_API$9, SELECTOR_DATA_TOGGLE$11, (event) => {
	event.preventDefault();
	const button = event.target.closest(SELECTOR_DATA_TOGGLE$11);
...
const EVENT_SLID = `slid${EVENT_KEY$17}`;
const EVENT_KEYDOWN$1 = `keydown${EVENT_KEY$17}`;
const EVENT_MOUSEENTER$2 = `mouseenter${EVENT_KEY$17}`;
const EVENT_MOUSELEAVE$1 = `mouseleave${EVENT_KEY$17}`;
const EVENT_POINTERDOWN$1 = `pointerdown${EVENT_KEY$17}`;
const EVENT_LOAD_DATA_API$3 = `load${EVENT_KEY$17}${DATA_API_KEY$11}`;
const EVENT_CLICK_DATA_API$8 = `click${EVENT_KEY$17}${DATA_API_KEY$11}`;
...
var Carousel = class Carousel extends BaseComponent {
	...
	dispose() {
		...
		EventHandler.off(this._viewport, EVENT_KEY$17);
		super.dispose();
	}
	_configAfterMerge(config) {
		if (![
			...
			ENDS_WRAP,
			ENDS_LOOP
		].includes(config.ends)) config.ends = Default$17.ends;
		return config;
	}
	_initialActiveIndex() {
	...
	_preserveFocus(atStart, atEnd) {
		...
		const fallback = (focusedIsPrev ? !atEnd : !atStart) ? opposite[0] : null;
		if (fallback) {
			fallback.focus({ preventScroll: true });
			return;
		}
		if (!this._viewport.hasAttribute("tabindex")) this._viewport.setAttribute("tabindex", "-1");
		this._viewport.focus({ preventScroll: true });
	}
	_setControlsDisabled(controls, disabled) {
		for (const control of controls) if ("disabled" in control) control.disabled = disabled;
	...
	_itemInterval(index = this._activeIndex) {
		...
		const interval = item ? Number.parseInt(item.getAttribute("data-cx-interval") ?? "", 10) : NaN;
...
*/
const NAME$19 = "chip";
const EVENT_KEY$16 = `.cx.chip`;
const DATA_API_KEY$10 = ".data-api";
const CLASS_NAME_ACTIVE$3 = "active";
const SELECTOR_DATA_TOGGLE$10 = "[data-cx-toggle=\"chip\"]";
const EVENT_CLICK_DATA_API$7 = `click${EVENT_KEY$16}${DATA_API_KEY$10}`;
const EVENT_CLOSE$1 = `close${EVENT_KEY$16}`;
const EVENT_TOGGLE$1 = `toggle${EVENT_KEY$16}`;
/**
* Class definition
*/
var Chip = class extends BaseComponent {
	...
	close() {
		if (EventHandler.trigger(this._element, EVENT_CLOSE$1).defaultPrevented) return;
	...
	toggle() {
		...
		EventHandler.trigger(this._element, EVENT_TOGGLE$1, { active: isActive });
	}
};
/**
* Data API implementation
*/
EventHandler.on(document, EVENT_CLICK_DATA_API$7, SELECTOR_DATA_TOGGLE$10, (event) => {
	event.preventDefault();
	const chip = event.target.closest(SELECTOR_DATA_TOGGLE$10);
...
* Constants
*/
const NAME$18 = "chip-input";
const EVENT_KEY$15 = `.cx.chip-input`;
const DATA_API_KEY$9 = ".data-api";
const EVENT_ADD = `add${EVENT_KEY$15}`;
const EVENT_REMOVE = `remove${EVENT_KEY$15}`;
const EVENT_CHANGE$2 = `change${EVENT_KEY$15}`;
const EVENT_SELECT = `select${EVENT_KEY$15}`;
...
* Data API implementation
*/
EventHandler.on(document, `DOMContentLoaded${EVENT_KEY$15}${DATA_API_KEY$9}`, () => {
...
* Constants
*/
const NAME$17 = "collapse";
const EVENT_KEY$14 = `.cx.collapse`;
...
const EVENT_KEYDOWN_DATA_API = `keydown${EVENT_KEY$13}${DATA_API_KEY$7}`;
const EVENT_KEYUP_DATA_API = `keyup${EVENT_KEY$13}${DATA_API_KEY$7}`;
...
* Class definition
*/
var Menu = class Menu extends FloatingBase {
	static _openInstances = /* @__PURE__ */ new Set();
	constructor(element, config) {
		if (typeof computePosition === "undefined") throw new TypeError("Chassis CSS's menus require Floating UI (https://floating-ui.com)");
		super(element, config);
	...
	_getConfig(config) {
		...
		if (typeof config.reference === "object" && !isElement$1(config.reference) && typeof config.reference.getBoundingClientRect !== "function") throw new TypeError(`${NAME$16.toUpperCase()}: Option "reference" provided type "object" without a required "getBoundingClientRect" method.`);
		return config;
	}
	_createFloating() {
	...
	async _applyFloatingPosition(reference, floating, placement, middleware, strategy = "absolute") {
		if (!floating.isConnected || !floating.classList.contains(CLASS_NAME_SHOW$5)) return null;
	...
	_setupSubmenuListeners() {
		if (!this._menu || !SelectorEngine.findOne(SELECTOR_SUBMENU, this._menu)) return;
		if (typeof window !== "undefined" && window.matchMedia && window.matchMedia("(hover: hover)").matches && (this._config.submenuTrigger === "hover" || this._config.submenuTrigger === "both")) {
	...
	_onSubmenuTriggerClick(event) {
		...
		this._closeSiblingSubmenus(submenuWrapper);
		this._openSubmenu(trigger, submenu, submenuWrapper);
		if (submenu.classList.contains(CLASS_NAME_STACKED)) requestAnimationFrame(() => {
			(SelectorEngine.findOne(SELECTOR_SUBMENU_BACK, submenu) || SelectorEngine.findOne(SELECTOR_VISIBLE_ITEMS$1, submenu))?.focus();
		});
	}
	...
	_trackMousePosition(event) {
		...
		while (this._hoverIntentSamples.length > 0 && now - this._hoverIntentSamples[0].t > 300) this._hoverIntentSamples.shift();
	}
	_isMovingTowardSubmenu(event, submenu) {
		if (this._hoverIntentSamples.length < 2) return false;
		const now = Date.now();
		let oldSample = null;
		for (let i = this._hoverIntentSamples.length - 1; i >= 0; i--) if (now - this._hoverIntentSamples[i].t >= 50) {
	...
	static dataApiKeydownHandler(event) {
		...
		if (!getToggleButton) {
			...
			while (enclosingSubmenu) {
				...
				if (!enclosingMenu || enclosingMenu === rootMenu) break;
				rootMenu = enclosingMenu;
				enclosingSubmenu = rootMenu.parentElement?.closest(SELECTOR_SUBMENU);
			}
			if (rootMenu !== this) getToggleButton = SelectorEngine.prev(rootMenu, SELECTOR_DATA_TOGGLE$8)[0] || SelectorEngine.next(rootMenu, SELECTOR_DATA_TOGGLE$8)[0] || SelectorEngine.findOne(SELECTOR_DATA_TOGGLE$8, rootMenu.parentNode);
...
EventHandler.on(document, EVENT_CLICK_DATA_API$5, SELECTOR_DATA_TOGGLE$8, function(event) {
	...
	if (event._chassisMenuToggled) return;
...
var Combobox = class Combobox extends BaseComponent {
	...
	dispose() {
		...
		EventHandler.off(this._menu, EVENT_KEY$12);
		EventHandler.off(this._toggle, EVENT_KEY$12);
		if (this._comboInput) EventHandler.off(this._comboInput, EVENT_KEY$12);
		if (this._searchInput) EventHandler.off(this._searchInput, EVENT_KEY$12);
		super.dispose();
	}
	_isShown() {
	...
	_addEventListeners() {
		...
		EventHandler.on(this._menu, `click${EVENT_KEY$12}`, SELECTOR_MENU_ITEM, (event) => {
		...
		EventHandler.on(this._toggle, `keydown${EVENT_KEY$12}`, (event) => {
			this._handleToggleKeydown(event);
		});
		EventHandler.on(this._menu, `keydown${EVENT_KEY$12}`, (event) => {
			this._handleMenuKeydown(event);
		});
		if (this._comboInput) {
			EventHandler.on(this._comboInput, `focus${EVENT_KEY$12}`, () => {
...
* Constants
*/
const NAME$14 = "datepicker";
const EVENT_KEY$11 = `.cx.datepicker`;
const DATA_API_KEY$5 = ".data-api";
const EVENT_CHANGE = `change${EVENT_KEY$11}`;
...
EventHandler.on(document, `DOMContentLoaded${EVENT_KEY$11}${DATA_API_KEY$5}`, () => {
	for (const element of document.querySelectorAll(`${SELECTOR_DATA_TOGGLE$6}[data-cx-inline="true"]`)) Datepicker.getOrCreateInstance(element);
});
//#endregion
...
var DialogBase = class extends BaseComponent {
	...
	_showElement({ modal = true, preventBodyScroll = true } = {}) {
		...
		if (preventBodyScroll) document.body.classList.add(CLASS_NAME_OPEN);
...
EventHandler.on(document, EVENT_CLICK_DATA_API$2, SELECTOR_DATA_TOGGLE$5, function(event) {
	...
	if (currentDialog && currentDialog !== target) {
		...
		newDialog.show(this);
		EventHandler.one(target, `shown${EVENT_KEY$10}`, () => {
...
var Drawer = class extends DialogBase {
	...
	_initSwipe() {
		...
		else if (element.classList.contains("drawer-end")) swipeConfig[isRTL$1() ? "leftCallback" : "rightCallback"] = () => this.hide();
		else swipeConfig[isRTL$1() ? "rightCallback" : "leftCallback"] = () => this.hide();
...
const EVENT_KEY$6 = `.cx.notification`;
...
const EVENT_KEY$5 = `.cx.otp-input`;
...
const EVENT_KEY$4 = `.cx.scrollspy`;
...
var Strength = class extends BaseComponent {
	...
	_updateUI(strength) {
		if (strength) this._element.dataset.cxStrength = strength;
		else delete this._element.dataset.cxStrength;
